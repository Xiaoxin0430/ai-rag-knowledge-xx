...
const API_ORIGIN = window.API_ORIGIN || 'http://localhost:8090';
const API_RAG_BASE = `${API_ORIGIN}/api/v1/rag`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const loadRagOptions = () => {
        ...
        fetch(`${API_RAG_BASE}/query_rag_tag_list`)
...
function getStreamContent(data) {
    return data?.result?.output?.content || data?.output?.content || data?.content || '';
}

function getFinishReason(data) {
    return data?.result?.metadata?.finishReason
        || data?.result?.output?.properties?.finishReason
        || data?.metadata?.finishReason
        || '';
}

function getVisibleText(content) {
    return content
...
function appendMessage(content, isAssistant = false, saveToStorage = true) {
 ...
 function startEventStream(message) {
    ...
    params.append('model', aiModelModel);
    if (ragTag) {
        params.append('ragTag', ragTag);
    }

    const url = `${API_RAG_BASE}/chat?${params.toString()}`;
    ...
    currentEventSource.onmessage = function(event) {
        try {
            ...
            const newContent = getStreamContent(data);
            if (newContent) {
                ...
                tempMessageDiv.textContent = getVisibleText(accumulatedContent);
                chatArea.scrollTop = chatArea.scrollHeight;
            }

            if (getFinishReason(data) === 'STOP') {
                currentEventSource.close();

                // 流式传输完成后进行最终渲染
                const finalContent = getVisibleText(accumulatedContent) || '没有生成内容。';
                if (!tempMessageDiv) {
                    tempMessageDiv = document.createElement('div');
                    tempMessageDiv.className = 'max-w-4xl mx-auto mb-4 p-4 rounded-lg bg-gray-100 markdown-body relative';
                    chatArea.appendChild(tempMessageDiv);
    ...
    currentEventSource.onerror = function(error) {
        ...
        currentEventSource.close();
        if (!accumulatedContent && !tempMessageDiv) {
            appendMessage('连接中断，请检查后端服务或稍后重试。', true);
        }
    };
}
